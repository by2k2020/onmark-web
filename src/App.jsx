...
const OnMarkWeb = () => {
  ...
  // 기능 5: 라이브 데모 (슬라이더 기능 구현) - NEW
  const [sliderPosition, setSliderPosition] = useState(50);
  const [isResizing, setIsResizing] = useState(false);
  const [showWatermarkData, setShowWatermarkData] = useState(false); // 워터마크 데이터 시각화 여부
  ...
  const handleEmergencyAnalysis = () => {
    ...
    setIsAnalyzing(true);
    showToast('긴급 분석 요청이 접수되었습니다.', 'info');
    setTimeout(() => {
        setIsAnalyzing(false);
        showToast('분석 완료: 유포자 추적 데이터가 갱신되었습니다.', 'success');
    }, 3000);
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 font-sans selection:bg-blue-500 selection:text-white overflow-x-hidden relative">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
          <div className="bg-slate-900 w-full max-w-lg rounded-2xl border border-slate-700 shadow-2xl overflow-hidden relative">
            ...
            <div className="p-8 text-center">
              ...
              {uploadStep === 'idle' && (
                <div 
                  className="border-2 border-dashed border-slate-700 rounded-xl p-10 hover:border-blue-500 hover:bg-slate-800/50 transition-all cursor-pointer group"
      ...
      <nav className="sticky top-0 z-40 bg-slate-950/80 backdrop-blur-md border-b border-slate-800">
          ...
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="hidden md:flex space-x-8 items-center text-sm font-medium text-slate-300">
              ...
              <button 
                ...
                className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg transition-all shadow-lg shadow-blue-900/20 text-sm font-semibold"
              >
                베타 참여하기
              </button>
            </div>
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-slate-900 border-b border-slate-800 p-4 space-y-4">
            ...
            <button 
              onClick={() => { handleOpenUploadModal(); setIsMenuOpen(false); }}
      ...
      <section id="hero" className="relative pt-20 pb-32 px-4 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <button 
              onClick={handleOpenUploadModal}
              className="w-full sm:w-auto px-8 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-semibold shadow-lg shadow-blue-900/30 transition-all flex items-center justify-center gap-2"
            >
              <Smartphone size={18} />
              무료로 사진 보호하기
            </button>
            <button 
              onClick={() => document.getElementById('police-test').scrollIntoView({ behavior: 'smooth' })}
              className="w-full sm:w-auto px-8 py-3 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl font-semibold transition-all flex items-center justify-center gap-2"
            >
              <Activity size={18} />
              테스트 현황 보기
            </button>
      ...
      {/* Technology Section - Live Demo Functional Upgrade */}
      <section id="technology" className="py-24 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            {/* Interactive Demo UI - Slider Implementation */}
            <div className="bg-slate-900 rounded-3xl p-6 border border-slate-800 shadow-2xl relative select-none">
              ...
              >
                ...
                <div className="absolute inset-0 w-full h-full">
                  ...
                  />
                  {/* Watermark Data Overlay (Visible only when toggled) */}
                  {showWatermarkData && (
                    <div className="absolute inset-0 bg-blue-900/30 mix-blend-overlay">
                       ...
                       <div className="absolute bottom-4 right-4 bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg animate-pulse">
                         DATA DETECTED
                       </div>
                    </div>
                  )}
                  <div className="absolute top-4 right-4 bg-blue-600/90 text-white text-xs px-2 py-1 rounded font-bold shadow-sm">
                    Protected
                  </div>
                </div>
                ...
                <div 
                  className="absolute inset-0 w-full h-full border-r-2 border-white bg-slate-800"
                  style={{ width: `${sliderPosition}%`, overflow: 'hidden' }}
                >
                  <img 
                    src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?fit=crop&w=800&q=80" 
                    alt="Original" 
                    className="absolute top-0 left-0 max-w-none h-full object-cover grayscale md:grayscale-0"
                    style={{ width: demoContainerRef.current ? demoContainerRef.current.clientWidth : '100%' }} 
                  />
                   <div className="absolute top-4 left-4 bg-slate-700/90 text-white text-xs px-2 py-1 rounded font-bold shadow-sm">
                    Original
                  </div>
                </div>
                ...
                {/* Slider Handle */}
                <div 
                  className="absolute top-0 bottom-0 w-1 bg-white cursor-ew-resize shadow-[0_0_15px_rgba(0,0,0,0.5)] flex items-center justify-center z-20"
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex justify-center gap-3">
                  <button 
                    ...
                    className="px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-sm font-medium transition-all flex items-center gap-2 border border-slate-700"
                  >
                    원본만 보기
                  </button>
                  <button 
                    onClick={() => {
                       setShowWatermarkData(!showWatermarkData);
                       setSliderPosition(100); // 자동으로 Protected 쪽으로 이동
                    }}
              ...
              <p className="text-center text-xs text-slate-500 mt-4">
                * 슬라이더를 움직여보세요. 육안으로는 차이를 구분할 수 없습니다 (SSIM 0.958)
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="p-6 md:p-8">
                <div className="flex justify-between items-center mb-8">
                  ...
                  <button 
                    ...
                    className={`px-3 py-1 rounded text-xs font-medium border transition-all ${isAnalyzing ? 'bg-red-500 text-white border-red-500 animate-pulse' : 'bg-red-500/10 text-red-400 border-red-500/20 hover:bg-red-500/20'}`}
                  >
                    {isAnalyzing ? '분석 진행 중...' : '긴급 분석 요청'}
                  </button>
                </div>
                ...
                <div className="bg-slate-900 rounded-xl border border-slate-800 p-6 mb-6">
                  ...
                  <div className="space-y-3">
                     <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                        <div className={`h-full bg-blue-500 transition-all duration-1000 ${isAnalyzing ? 'w-[85%]' : 'w-[20%]'}`}></div>
                     </div>
                     <div className="flex justify-between text-xs text-slate-400">
                        <span>워터마크 복구율: {isAnalyzing ? '95%' : '대기 중'}</span>
                        <span>{isAnalyzing ? '유포 경로 추적 완료' : '분석 대기'}</span>
                     </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-slate-900 p-4 rounded-xl border border-slate-800">
                     ...
                     <div className={`font-bold transition-colors ${isAnalyzing ? 'text-red-400' : 'text-slate-500'}`}>
                       {isAnalyzing ? '탐지됨 (Face Swap)' : '분석 전'}
                     </div>
                  </div>
                </div>
              </div>
        ...
        <form onSubmit={handleFeedbackSubmit} className="bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-xl">
           <div className="flex flex-col gap-4 text-left">
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-end">
                ...
                <button 
                  type="submit"
                  className={`px-6 py-3 rounded-lg font-semibold flex items-center justify-center gap-2 transition-all ${feedbackSent ? 'bg-green-600 text-white' : 'bg-blue-600 hover:bg-blue-500 text-white'}`}
                >
